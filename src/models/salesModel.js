...
const update = async (id, arr, oldProductId) => {
  await Promise.all(arr
    .map(async (e, i) => {
      await connection
        .execute(`UPDATE sales_products SET product_id = ?
        , quantity = ? WHERE sale_id = ? AND product_id = ?`,
          [e.productId, e.quantity, id, oldProductId[i].productId]);
    }));
